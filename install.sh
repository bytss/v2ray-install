...
_load() {
	local _dir="/etc/v2ray/bytss/v2ray/src/"
	. "${_dir}$@"
}
_sys_timezone() {
	IS_OPENVZ=
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		mkdir -p /etc/v2ray/bytss/v2ray
		cp -rf $(pwd)/* /etc/v2ray/bytss/v2ray
	else
		pushd /tmp
		git clone https://github.com/bytss/v2ray -b "$_gitbranch" /etc/v2ray/bytss/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/bytss/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/bytss/v2ray/config/backup.conf $backup
